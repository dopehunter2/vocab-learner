            ...
            <ScrollView style={styles.screenContainer} contentContainerStyle={styles.scrollContentContainer}>
                ...
                {wordInfo && (
                    <View style={styles.resultsContainer}>
                        <Text style={styles.sectionTitle}>Translations for "{wordInfo.originalWord}" ({wordInfo.identifiedLang})
                            <TouchableOpacity onPress={() => playText(wordInfo.originalWord, wordInfo.identifiedLang || undefined)} style={styles.speakerButton}>
                                <Ionicons name="volume-medium-outline" size={22} color={ACCENT_COLOR} />
                            </TouchableOpacity>
                        </Text>
                        {wordInfo.generalExamples && wordInfo.generalExamples.length > 0 && (
                            <View style={styles.exampleSection}>
                                <Text style={styles.exampleTitle}>Examples for "{wordInfo.originalWord}":</Text>
                                {wordInfo.generalExamples.map((ex, i) => (
                                    <Text key={`genex-${i}`} style={styles.exampleTextListItem}>• {ex}</Text>
                                ))}
                            </View>
                        )}
                                ...
                                {wordInfo.translations.map((transObj: AiTranslation, index) => {
                                    ...
                                    return (
                                        <View key={`chip-container-${index}`} style={styles.translationChipOuterContainer}>
                                            <TouchableOpacity 
                                                key={`chip-${index}`} 
                                                style={[styles.translationChip, isSelected && styles.selectedChip]}
                                                onPress={() => handleTranslationPress(transObj)}
                                                onLongPress={() => handleShowBilingualExample(transObj)} // Long press to show example
                                            >
                                                ...
                                                {transObj.examplesForThisTranslation && transObj.examplesForThisTranslation.length > 0 && (
                                                    <View style={styles.translationExampleContainer}>
                                                        {transObj.examplesForThisTranslation.slice(0,1).map((ex, exIdx) => (
                                                            <Text key={`transChipEx-${index}-${exIdx}`} style={styles.translationExampleText}>e.g. {ex}</Text>
                                                        ))}
                                                    </View>
                                                )}
                                            </TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    translationChipOuterContainer: {
        marginRight: 8,
        marginBottom: 8,
        // alignItems: 'center', // Keep or remove based on whether chip alone needs centering or if it's part of a row
    },
    // Styles for Modal
    modalOverlay: {
        flex: 1,
