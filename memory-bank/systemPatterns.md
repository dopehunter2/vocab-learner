...
- **File Operations Service**: Using a dedicated `fileService.ts` to abstract file system interactions (`expo-file-system`), sharing (`expo-sharing`), and document picking (`expo-document-picker`) for the import/export feature.
- **Cross-Platform Development**: Using React Native with Expo, now building with the **New Architecture (Fabric) enabled**.
...
- **(Removed) External Dictionary API Integration**: The pattern for using an external dictionary API has been removed.
- **Notification Scheduling**: For study reminders, the app uses `expo-notifications`. The system schedules only one notification at a time, corresponding to the next closest due card. This notification is cancelled and recalculated after every review session (or other data-modifying action) to ensure the reminder is always accurate based on the latest FSRS data.
- **Database Migration**: A database migration pattern is used to handle schema changes without data loss. 