...
- **Database Migration**: To handle app updates safely, a database migration pattern is used. This involves assigning a version number to the database schema (`PRAGMA user_version`). When an app update requires a schema change, a corresponding migration script is created to alter the existing database structure without data loss. An engine in `database.ts` checks the version on startup and applies any necessary migrations sequentially. 