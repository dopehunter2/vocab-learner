...
import { scheduleNextReviewNotification, cancelAllNotifications } from '../services/notificationService';
...
export const useVocabularyStore = create<VocabularyState>((set, get) => ({
    ...
    exportData: async () => {
        try {
            ...
            // We can add more data to the export object if needed in the future
            const exportObject = {
                version: 1,
                exportDate: new Date().toISOString(),
                cards: allCards,
            };
            await exportDataToFile(exportObject);
            // Optionally, provide feedback to the user upon successful export initiation
        } catch (error) {
            console.error('Export failed:', error);
            // Optionally, provide feedback to the user about the failure
            throw error; // Re-throw to be caught in the UI layer if needed
        }
    },
